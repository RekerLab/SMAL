#!/usr/bin/env bash
# Short-term Memory Active Learning (SMAL) on PAMPA_NCATS with RandomForest/Morgan.
#
# Reproduces results-initial/smal/smal-PAMPA_NCATS-*
...
F_MINS=(420 642 641 888 780)  # f_min_train_size per error rate
MAX_ITER=1014
...
for i in "${!ERRS[@]}"; do
  ...
  for METHOD in "${FORGET_METHODS[@]}"; do
    for SEED in $(seq 0 19); do
      SAVE_DIR="${OUT_ROOT}/smal-PAMPA_NCATS-${ERR}-${METHOD}-${SEED}"
      rm -rf "${SAVE_DIR}"
      molalkit_run \
        --data_path "${PKG_DATA}/PAMPA_NCATS.csv" --smiles_columns SMILES --targets_columns Y --task_type binary \
