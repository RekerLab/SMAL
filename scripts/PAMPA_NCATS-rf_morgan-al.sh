#!/usr/bin/env bash
# Active learning on PAMPA_NCATS with RandomForest/Morgan.
#
# Reproduces results-initial/active_learning/al-PAMPA_NCATS-*
...
for ERR in 0 10 20 30 40; do
  ...
  for SEED in $(seq 0 19); do
    SAVE_DIR="${OUT_ROOT}/al-PAMPA_NCATS-${ERR}-${SEED}"
    rm -rf "${SAVE_DIR}"
    molalkit_run \
