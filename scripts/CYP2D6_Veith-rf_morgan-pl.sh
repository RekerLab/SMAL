#!/usr/bin/env bash
# Passive learning on CYP2D6_Veith with RandomForest/Morgan.
#
# Reproduces results-initial/passive_learning/pl-CYP2D6_Veith-*
...
for ERR in 0 10 20 30 40; do
  ...
  for SEED in $(seq 0 19); do
    SAVE_DIR="${OUT_ROOT}/pl-CYP2D6_Veith-${ERR}-${SEED}"
    rm -rf "${SAVE_DIR}"
    molalkit_run \
