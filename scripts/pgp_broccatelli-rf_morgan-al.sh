#!/usr/bin/env bash
# Active learning on pgp_broccatelli with RandomForest/Morgan.
#
# Reproduces results-initial/active_learning/al-pgp_broccatelli-*
...
for ERR in 0 10 20 30 40; do
  ...
  for SEED in $(seq 0 19); do
    SAVE_DIR="${OUT_ROOT}/al-pgp_broccatelli-${ERR}-${SEED}"
    rm -rf "${SAVE_DIR}"
    molalkit_run \
