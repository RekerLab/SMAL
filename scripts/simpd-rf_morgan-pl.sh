#!/usr/bin/env bash
# Passive learning on all SIMPD ChEMBL datasets with RandomForest/Morgan.
#
# Reproduces results-simpd/passive_learning/pl-CHEMBL*-*
# (9900 molalkit_run invocations: 99 datasets x 5 error rates x 20 seeds).
set -euo pipefail

OUT_ROOT="${OUT_ROOT:-./results-simpd/passive_learning}"
...
for DATA_PATH in "${DATASETS[@]}"; do
  ...
  for ERR in 0 10 20 30 40; do
    ...
    for SEED in $(seq 0 19); do
      SAVE_DIR="${OUT_ROOT}/pl-${DATASET}-${ERR}-${SEED}"
      rm -rf "${SAVE_DIR}"
      molalkit_run \
        "${DATA_FLAGS[@]}" \
