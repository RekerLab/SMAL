#!/usr/bin/env bash
# Short-term Memory Active Learning (SMAL) on CYP3A4_Veith with RandomForest/Morgan.
#
# Reproduces results-initial/smal/smal-CYP3A4_Veith-*
...
F_MINS=(4514 5196 5416 4636 3902)  # f_min_train_size per error rate
MAX_ITER=6161
...
for i in "${!ERRS[@]}"; do
  ...
  for METHOD in "${FORGET_METHODS[@]}"; do
    for SEED in $(seq 0 19); do
      SAVE_DIR="${OUT_ROOT}/smal-CYP3A4_Veith-${ERR}-${METHOD}-${SEED}"
      rm -rf "${SAVE_DIR}"
      molalkit_run \
        --data_public CYP3A4_Veith \
