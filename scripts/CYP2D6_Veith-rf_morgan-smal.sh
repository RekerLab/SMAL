#!/usr/bin/env bash
# Short-term Memory Active Learning (SMAL) on CYP2D6_Veith with RandomForest/Morgan.
#
# Reproduces results-initial/smal/smal-CYP2D6_Veith-*
...
F_MINS=(1485 2070 2710 4272 4069)  # f_min_train_size per error rate
MAX_ITER=6562
...
for i in "${!ERRS[@]}"; do
  ...
  for METHOD in "${FORGET_METHODS[@]}"; do
    for SEED in $(seq 0 19); do
      SAVE_DIR="${OUT_ROOT}/smal-CYP2D6_Veith-${ERR}-${METHOD}-${SEED}"
      rm -rf "${SAVE_DIR}"
      molalkit_run \
        --data_public CYP2D6_Veith \
